-- 导数据后访问以下网址转换会员编号
-- http://61.145.230.82:81/convert.php
--导数据SQL
...
(account_field1, account_name, account_status, account_addr, account_post, account_phone, account_field2, account_fax,
 account_industry, industry_desc, account_field3, account_field4, account_field5, company_nature, account_field6, account_birthday, account_field9, remark,
 update_man, update_time, create_time, account_url, account_email, account_field10, account_field12, account_field14, account_type, account_field15, deleted, field3, field4)
select 
unix_timestamp(HY_REG), HY_NAM, HY_STA, HY_ADD, HY_POT, HY_TEL, HY_TE2, HY_FAX, 
HY_TRA, HY_PRO, HY_CUR, HY_AMO, HY_CAP, HY_TYP, HY_NAT,  unix_timestamp(`HY_JOI`), HY_COD, HY_MEM, 
HY_CXM, unix_timestamp(HY_CRQ), unix_timestamp(HY_CRQ), hy_url, HY_email, 理事企业, 荣誉史, FaxType, MemberType, CorperationCode, 0, HY_PRE, HY_MAN
from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80);
update TD_OA.`crm_account` set account_type='B-游离会员' where account_status = 80;
update TD_OA.`crm_account` set account_url = replace(account_url, 'http://', '')
WHERE `account_url` LIKE '%http://%'
order by id;
...
from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and ((hy_con is not null and hy_con <> '') or (hy_contel is not null and hy_contel <> '') or (hy_concel is not null and hy_concel <> ''));
-- 行政联系人
insert into TD_OA.`crm_account_contact` 
(`create_time`, `update_time`, `create_man`, `create_man_text`, `update_man`, `update_man_text`, `owner`, `owner_dept`, `create_dept`, `deleted`, `account_id`, `account_id_text`, `contact_position`, contact_phone, `contact_mobile`, `contact_name`)
select unix_timestamp(now()), unix_timestamp(now()), 'admin', '系统管理员', 'admin', '系统管理员', 'admin', 1, 
...
from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and ((hy_con1 is not null and hy_con1 <> '') or (hy_contel1 is not null and hy_contel1 <> '') or (hy_concel1 is not null and hy_concel1 <> ''));
-- 财务联系人
insert into TD_OA.`crm_account_contact` 
(`create_time`, `update_time`, `create_man`, `create_man_text`, `update_man`, `update_man_text`, `owner`, `owner_dept`, `create_dept`, `deleted`, `account_id`, `account_id_text`, `contact_position`, contact_phone, `contact_mobile`, `contact_name`)
select unix_timestamp(now()), unix_timestamp(now()), 'admin', '系统管理员', 'admin', '系统管理员', 'admin', 1, 
...
from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and ((hy_con2 is not null and hy_con2 <> '') or (hy_contel2 is not null and hy_contel2 <> '') or (hy_concel2 is not null and hy_concel2 <> ''));
-- HR联系人
insert into TD_OA.`crm_account_contact` 
(`create_time`, `update_time`, `create_man`, `create_man_text`, `update_man`, `update_man_text`, `owner`, `owner_dept`, `create_dept`, `deleted`, `account_id`, `account_id_text`, `contact_position`, contact_phone, `contact_mobile`, `contact_name`)
select unix_timestamp(now()), unix_timestamp(now()), 'admin', '系统管理员', 'admin', '系统管理员', 'admin', 1, 
...
from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and ((hy_con3 is not null and hy_con3 <> '') or (hy_contel3 is not null and hy_contel3 <> '') or (hy_concel3 is not null and hy_concel3 <> ''));
-- 销售联系人
insert into TD_OA.`crm_account_contact` 
(`create_time`, `update_time`, `create_man`, `create_man_text`, `update_man`, `update_man_text`, `owner`, `owner_dept`, `create_dept`, `deleted`, `account_id`, `account_id_text`, `contact_position`, contact_phone, `contact_mobile`, `contact_name`)
select unix_timestamp(now()), unix_timestamp(now()), 'admin', '系统管理员', 'admin', '系统管理员', 'admin', 1, 
...
from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and ((hy_con4 is not null and hy_con4 <> '') or (hy_contel4 is not null and hy_contel4 <> '') or (hy_concel4 is not null and hy_concel4 <> ''));
-- 法定代表人/董事长
--insert into TD_OA.`crm_account_contact` 
...
--from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and HY_PRE is not null and HY_PRE <> '';
-- 总经理
--insert into TD_OA.`crm_account_contact` 
--(`create_time`, `update_time`, `create_man`, `create_man_text`, `update_man`, `update_man_text`, `owner`, `owner_dept`, `create_dept`, `deleted`, `account_id`, `account_id_text`, `contact_position`, `contact_name`)
--select unix_timestamp(now()), unix_timestamp(now()), 'admin', '系统管理员', 'admin', '系统管理员', 'admin', 1, 1, 0, 
--(select id from TD_OA.crm_account where account_name = HY_NAM limit 1), HY_NAM, '总经理', HY_MAN 
--from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80) and HY_MAN is not null and HY_MAN <> '';


-- 导入联系记录
--delete from wsxh2012.ContactRecord where memberid not in (select id from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80));
...
where memberid in (select id from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80))
;



-- 导入会费表
--delete from wsxh2012.huifei where id not in (select id from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80));
...
`account_id`,  `collection_date`, `collection_amount`, `hd_collection_man`, `hd_collection_man_text`, remark, `salepay_title`, `salepay_code`)
select 
unix_timestamp(h.HF_CRQ), unix_timestamp(h.HF_CRQ), h.HF_CXM, h.HF_CXM, h.HF_CXM, h.HF_CXM, h.HF_CXM, 1, 1, 0, 
h.id, unix_timestamp(h.HF_DAT), h.HF_AMO, h.HF_CXM, h.HF_CXM, HF_GIV, h.HF_YEA, h.HF_COD
from wsxh2012.huifei h
where h.HF_AMO > 0 and h.HF_YEA > 0
and id in (select id from wsxh2012.xhhy where hy_sta in (10, 11, 60, 80));
...
where s.salepay_field2 is null and s.salepay_field2_text is null;

update TD_OA.crm_salepay set 
salepay_field2 = 7,
salepay_field2_text = '往年会费'
where salepay_field2 is null and salepay_field2_text is null;
  
-- 更新会费表的会员ID、名称；根据id查询，没有id的根据会费卡号
update TD_OA.crm_salepay set `account_id_text` = (select hy_nam from wsxh2012.xhhy where id = TD_OA.crm_salepay.account_id limit 1) where account_id_text is null;
